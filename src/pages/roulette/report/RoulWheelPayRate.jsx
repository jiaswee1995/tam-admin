...
import { postGetWheelPayRate } from '../../../store/ducks/roulette/actions'

const { Option } = Select;

class RoulWheelPayRate extends Component{
    constructor(){
        super();
        this.state = {
            cd: 60,
    ...
    getTableData = () => {
        ...
        data.wheel_code = this.state.wheel_code;
        data.order_by = this.state.orderBy;
        data.order_type = this.state.orderType;
        this.props.postGetWheelPayRate(data);
    }
    ...
    handleChange = (page, pageSize, sorter) => {
        ...
        let order_by  = sorter.column !== undefined ? sorter.field : "wheel_code";
    ...
    UNSAFE_componentWillReceiveProps(nextProps, nextContext) {
        if(nextProps.roul_pay_rate !== this.props.roul_pay_rate){
            if(nextProps.roul_pay_rate.statusCode === 200){
                this.setState({
                    loadingTable: false,
                    data: nextProps.roul_pay_rate.data,
                    total_records: nextProps.roul_pay_rate.total_records,
                })
            } else {
                this.setState({
                    data: [],
                ...
                message.error(nextProps.roul_pay_rate.msg)
            }
         }
...
const mapStateToProps = (state) => {
    ...
    return {
        roul_pay_rate : roulette.roul_pay_rate
    }
};
...
export default connect (mapStateToProps, mapDispatchToProps) (RoulWheelPayRate);