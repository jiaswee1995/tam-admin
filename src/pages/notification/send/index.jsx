...
import { postLanguageList, postMoudleList, postNotificationSend } from '../../../store/ducks/notification/actions';
import connect from "react-redux/es/connect/connect";
import { Tabs, Row, Col, Select, Button, message, Modal, Spin } from 'antd';
...
class NotificationSend extends Component{
    ...
    sendNotification = () => {
        if(this.state.module_id === 0){
            message.warn("Please Select Notification Module");
        } else {
            confirm({
                ...
                content: '',
                onOk : () => {
                  this.proceedSend(this.state.module_id);
                },
                onCancel() {
                  console.log('Cancel');
                },
    ...
    proceedSend = (moduleID) => {
        ...
        if(failed === 0){
            this.setState({
                ...
                const data = ps;
                this.props.postNotificationSend(data);
    ...
    render() {
        ...
        const { langList, moduleList, module_id, loading } = this.state;
        ...
        return (
            <PageContent page_title={t('notification.send_notification')} add_button_url="">
...
export default connect (mapStateToProps, mapDispatchToProps) (NotificationSend);