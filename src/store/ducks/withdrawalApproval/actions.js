...
export const postSecondWithdrawalApproval = (data) => dispatch => {
    axios.post(process.env.REACT_APP_SLOT_URL + '/api/v1/admin/wallet/approval', data , {
        headers: {
        ...
        dispatch ({
            type: type.POST_SECONDWITHDRAWALAPPROVAL,
...
export const postWithdrawalApproval = (data) => dispatch => {
    axios.post(process.env.REACT_APP_SLOT_URL + '/api/v1/admin/wallet/withdrawal/before/approval', data , {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': sessionStorage.getItem("accessToken")
       }
        ...
        dispatch ({
            type: type.POST_WITHDRAWALAPPROVAL,
            payload: response.data
        })
    }).catch (error => {
        dispatch ({
